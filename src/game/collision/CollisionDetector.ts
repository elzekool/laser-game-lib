  ...
  private objectsCollide(
    ...
    for (let i = 0; i < obj1IntersectionLines.length; i++) {
      for (let j = 0; j < obj2IntersectionLines.length; j++) {
        if (
          linesIntersect(obj1IntersectionLines[i], obj2IntersectionLines[j])
        ) {
          return true;
        }
      }
